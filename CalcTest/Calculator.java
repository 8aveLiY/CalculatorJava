...
class Calculator {
    ...
    public static String parse(String expression) throws Exception {
        ...
        if (operands.length != 2) throw new Exception("Нужны 2 числа");
        oper = detectOperation(expression);
        if (oper == null) throw new Exception("Неподдерживаемая математическая операция");
        if (Rom.isRoman(operands[0]) && Rom.isRoman(operands[1])) {
            num1 = Rom.convertToArabian(operands[0]);
            num2 = Rom.convertToArabian(operands[1]);
            isRoman = true;
        }
        else if (!Rom.isRoman(operands[0]) && !Rom.isRoman(operands[1])) {
